...
const ProductFinishes = () => {
    ...
    const [productFinishes, setProductFinishes] = useState<any>();
    ...
    const navAction = (type: any, record?: any) => {
        if (type === 'edit') {
            navigate(`/product-finishes/edit/${record.key}`)
        }

        if (type === 'add') {
            navigate(`/product-finishes/add`)
        }
    }
        ...
        try {
            fetch(`${import.meta.env.VITE_API_KEY}/all-finishes`, {
                ...
                .then((data) => {
                    ...
                    setProductFinishes([...updatedData]);
                }
                );
        } catch (error) {
    ...
    const deletion = (record?: any) => {
        ...
        setConfirmation({
            ...
            buttonText: 'Confirm',
            action: async () => {
                fetch(`${import.meta.env.VITE_API_KEY}/delete-finish/${record.id}`, {
                    method: 'POST',
                    headers: {
                    ...
                    .catch((error) => {
                        console.log('Delete Finish error:', error);
    ...
    return (
        <>
            ...
            <div>
                <Table dataSource={productFinishes} pagination={pagination}
                    onChange={handleTableChange}
                    rowKey='id'
                >
                    <Column title="Name" dataIndex="name" key="finishName" />
